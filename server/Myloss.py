...
class L_spa(nn.Module):
    ...
    def forward(self, org, enhance):
        device = org.device  # Ensure compatibility with the device of input tensors
        b, c, h, w = org.shape
        ...
        weight_diff = torch.max(torch.tensor(1.0, device=device) +
                                10000 * torch.min(org_pool - torch.tensor(0.3, device=device),
                                                  torch.tensor(0.0, device=device)),
                                torch.tensor(0.5, device=device))
        E_1 = torch.mul(torch.sign(enhance_pool - torch.tensor(0.5, device=device)), enhance_pool - org_pool)
...
class L_exp(nn.Module):
    ...
    def forward(self, x):
        ...
        mean = self.pool(x)
        d = torch.mean(torch.pow(mean - torch.tensor(self.mean_val, device=device), 2))
        return d
